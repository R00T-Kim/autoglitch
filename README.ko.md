...
AUTOGLITCH는 **하드웨어 보안 연구용 폐쇄 루프(closed-loop) 자율 글리칭 프레임워크**입니다.
실험 오케스트레이션, Bayesian/RL 최적화, 하드웨어 실행, 관측/분류, primitive 매핑,
재현성 리포팅을 하나의 워크플로우로 묶어 fault injection 실험을 소프트웨어적으로 체계화합니다.

**바로가기:** [핵심 특징](#핵심-특징) · [빠른 시작](#빠른-시작) · [하드웨어 온보딩](#하드웨어-온보딩) · [품질 게이트](#품질-게이트) · [문서](#문서)

> [!IMPORTANT]
> **2026년 3월 9일 기준**, 소프트웨어 품질 게이트는 모두 green 상태입니다.
...
> 소프트웨어 프레임워크와 mock/HIL 준비 환경으로서는 충분히 강하지만,
> 아직 실장비 현장 검증이 끝난 프로젝트라고 주장하지는 않습니다.
...
- **외부 backend 기준선**: 첫 USB backend로 `chipwhisperer-hardware`를 지원합니다.
- **agentic 제어 경로**: planner/policy 루프, eval-suite, 로컬 knowledge ingest/query 유틸리티.
- **런타임 선택형 컴포넌트**: observer / classifier / mapper를 plugin manifest 이름으로 선택할 수 있습니다.
- **재현성 산출물**: campaign summary, run manifest, artifact bundle, RL report,
  eval-suite report, JSONL trial log, decision trace.
- **backend benchmark 경로**: benchmark 실행이 이제 **backend × algorithm** 조합 비교를 지원합니다.
...
| ChipWhisperer backend | ✅ | 첫 외부 USB backend로 `chipwhisperer-hardware` 통합 |
...
| Artifact bundle | ✅ | 모든 run이 `experiments/results/bundles/` 아래 재현성 bundle을 남길 수 있음 |
| Backend benchmark compare | ✅ | benchmark가 backend × algorithm 비교 리포트를 생성함 |
...
이 모드에서 `--serial-port`는 ChipWhisperer 뒤의 **target UART** 로 사용됩니다.
...
- `src/logging_viz/`: trial log, campaign summary, run manifest, artifact bundle, tracking helper
...
이제 런타임은 위 컴포넌트를 하드코딩된 클래스가 아니라 plugin registry를 통해 생성합니다.
실행 전 target 호환성도 함께 검증합니다.
...
Campaign summary에는 이제 실험 결과와 별도로 실행 상태 메트릭도 기록됩니다.
...
| [`docs/CHIPWHISPERER_ADAPTER_PLAN.md`](docs/CHIPWHISPERER_ADAPTER_PLAN.md) | ChipWhisperer backend 범위/상태 |
| [`docs/RESEARCH_POSITIONING.md`](docs/RESEARCH_POSITIONING.md) | 연구 포지셔닝과 차별점 |
