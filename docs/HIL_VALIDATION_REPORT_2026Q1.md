...
> 상태(2026-03-09): **실측 데이터 대기 중**. 아래 템플릿은 유지하되, 현재 확보된 근거는 software-only validation과 artifact-bundle 경로까지다.
> software gate(2026-03-09): `python -m compileall src tests`, `ruff check src tests`, `mypy src`, `pytest -q` → `127 passed`
> 참고: backend benchmark / artifact bundle / `chipwhisperer-hardware` 경로는 소프트웨어적으로 구현 완료되었지만, 실장비 측정 artifact는 아직 본 문서에 첨부되지 않았다.
...
실장비 투입 절차 자체는 [`docs/REAL_HARDWARE_CHECKLIST.md`](REAL_HARDWARE_CHECKLIST.md) 를 기준으로 수행한다.
...
7. 각 run에 대해 artifact bundle 경로 보관
...
- Artifact bundles: software path ready, lab bundles TBD
- Aggregate: TBD
- Gate pass/fail: TBD
...
- 다음 액션:
  ...
  - `artifact_bundle` / `bundle_manifest` 경로를 결과 표에 함께 남기기
